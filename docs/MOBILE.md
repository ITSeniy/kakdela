...
DEK (ключ шифрования стора) сейчас отдаёт `SoftwareKeyProvider` (`src/sealed.rs`) —
он хранит 32-байтный ключ файлом рядом со стором (тот же программный уровень, что
...
key, StrongBox если доступен) — подменяет ТОЛЬКО `KeyProvider` в `sealed.rs`,
крипто-ядро и историю не трогает. Сюда же сворачивается hardware-хранилище JWT
из T-100.

---

## Транспорт секретных чатов (T-102)

Слепой релей (store-and-forward очередь шифр-конвертов) + локальная история на
устройстве. Сервер хранит непрозрачный `ciphertext bytea` и удаляет его после
ack получателем (модель Signal); расшифровать не может.

- **Локальная история** `src-tauri/src/store/local_db.rs` — источник истины
  переписки. Зашифрованный снапшот сообщений через общий `src/sealed.rs` (тот же
  DEK, что у крипто-стора). НЕ SQLite/SQLCipher — последнее тянет C/boringssl и
  ломает pure-Rust Android-сборку; снапшота хватает на текст для 15-20 друзей.
- **Вид сообщения** (текст / read-receipt / typing) — это зашифрованный
  `SecretFrame` ВНУТРИ конверта (`ginzu/api-types.ts`), а НЕ серверный `msgType`.
  Сервер не видит даже характер трафика. `msgType` = тип ciphertext'а libsignal.
- **Pipeline** `src/features/secret/api.ts`: send/receive/read-receipt/typing,
  `startSecretChatListener` (WS `secret.envelope`/`ready` → последовательный слив
  inbox), публикация/долив prekey-бандла. Тесты истории: `cargo test --lib store`.
- **Wipe данных → история и ключи исчезают** (`<app_data>/kd-secret/`) — это
  device-bound by design, не баг.
